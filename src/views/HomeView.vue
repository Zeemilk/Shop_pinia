    ...
    <div class="row mt-2" v-else>
        <div class="col mb-2" v-for="(product, index) in search_products" :key="index">
            <div class="card" style="width: 19rem;">
                <img :src="product.image" class="card-img-top mx-auto border border-dark" alt="" style="width: 12rem; height: 12rem;">
                <div class="card-body">
                    <h5 class="card-title">{{ product.title }}</h5>
                    <p class="card-text">หมวดหมู่ {{ product.category }}</p>
                    <p class="card-text">ราคา {{ currencyTHB(product.price) }} บาท</p>
                    <routerLink :to="`/detail/${product.id}`" class="btn btn-primary mx-2">รายละเอียด</routerLink>
